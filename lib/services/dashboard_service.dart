...
class DashboardService {
  ...
  EvolutionData generateScoreEvolution() {
    ...
    final minY = _calculateMinY(series.map((s) => s.points.toDouble()).toList(),
        buffer: 5.0);
    final maxY = _calculateMaxY(series.map((s) => s.points.toDouble()).toList(),
        buffer: 5.0);
  ...
  EvolutionData generateGroupSizeEvolution() {
    ...
    final minY =
        _calculateMinY(series.map((s) => s.groupSize).toList(), buffer: 1.0);
    final maxY =
        _calculateMaxY(series.map((s) => s.groupSize).toList(), buffer: 1.0);
  ...
  DistributionData generateCategoryDistribution() {
    ...
    distribution.forEach((category, count) {
      percentageData[SessionConstants.categoryLabel(category)] =
          (count / total) * 100;
    });
  ...
  AdvancedStatsData generateAdvancedStats() {
    ...
    if (allSeries.isNotEmpty) {
      final oneHandCount =
          allSeries.where((s) => s.handMethod == HandMethod.oneHand).length;
      final twoHandsCount =
          allSeries.where((s) => s.handMethod == HandMethod.twoHands).length;
  ...
  (EvolutionData, EvolutionData) generateHandMethodEvolutionData(
    ...
    final handMethodLabel =
        handMethod == HandMethod.oneHand ? '1 main' : '2 mains';
    final pointsEvolution = _createEvolutionData(pointsData, pointsDates,
        pointsIndices, 'Points - $handMethodLabel', 'pts');
  ...
  Map<String, int> generateWeaponShotCounts(List<Weapon> rack) {
    final realized = _statsService.sessions
        .where((s) => s.status == SessionConstants.statusRealisee);
