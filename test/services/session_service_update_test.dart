...
void main() {
  group('SessionService update and planning methods', () {
    ...
    tearDown(() async {
      for (final name in ['sessions', 'exercises', 'app_preferences']) {
    ...
    test(
        'planFromExercise creates planned session with series from exercise consignes',
        () async {
      final service = SessionService();
    ...
    test('planFromExercise applies and persists the default caliber', () async {
      await PreferencesService().setDefaultCaliber('.45 ACP');
      final service = SessionService();
      final exercise = Exercise(
        id: 'default-caliber-exercise',
        name: 'Default caliber exercise',
      ...
      final plannedSession = await service.planFromExercise(exercise);
      final persistedSession = (await service.getAllSessions()).single;

      expect(plannedSession.caliber, '.45 ACP');
      expect(persistedSession.caliber, '.45 ACP');
    });

    test(
        'planFromExercise creates a default series for exercise without consignes',
        () async {
      final service = SessionService();
