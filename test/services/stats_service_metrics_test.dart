  ...
  group('StatsService metrics', () {
    ...
    test('bestSeriesByPoints returns highest', () {
      ...
      final s = ShootingSession(
        ...
        status: SessionConstants.statusRealisee,
        series: [
          Series(distance: 10, points: 5, groupSize: 20),
          Series(distance: 10, points: 50, groupSize: 10)
        ],
      );
    ...
    test('score stats exclude trial series and score gongs by fallen count',
        () {
      final now = DateTime(2025, 10, 7);
      final s = ShootingSession(
        date: now.subtract(const Duration(days: 1)),
        ...
        series: [
          ...
          Series(
            ...
            scoringMode: SeriesScoringMode.pointsZone,
          ),
          Series(
            distance: 25,
            points: 0,
      ...
      expect(stats.averagePointsLast30Days(), 55.5);
      expect(stats.bestSeriesByPoints()!.points, 91);
      expect(stats.movingAveragePoints(), [91, 55.5]);
      expect(
          stats
              .pointBuckets(bucketSize: 10, last30: false)
    ...
    test(
        'progressionPercent30Days computes positive when curr>prev and enough data',
        () {
      ...
      final prev = ShootingSession(
        ...
        series: List.generate(
            5, (i) => Series(distance: 10, points: 10, groupSize: 20)),
      );
      ...
      final curr = ShootingSession(
        ...
        status: SessionConstants.statusRealisee,
        series: List.generate(
            5, (i) => Series(distance: 10, points: 20, groupSize: 15)),
      );
      ...
      final s = ShootingSession(
        ...
        series: [
          ...
          Series(
              ...
              points: 14,
              groupSize:
                  26), // -> 10 (round half up -> 11? toStringAsFixed rounds half away)
        ],
      );
      final stats = StatsService([s], now: now);
    ...
    test('currentDayStreak counts consecutive days from most recent backwards',
      ...
      final s3 = ShootingSession(
        date: DateTime(2025, 10, 4), weapon: 'P', caliber: '22LR',
        status: SessionConstants.statusRealisee, // gap of one day (5th missing)
        series: [Series(distance: 10, points: 10, groupSize: 20)],
      );
    ...
    test('bestGroupSize returns smallest positive > 0', () {
      ...
      final s = ShootingSession(
        ...
        status: SessionConstants.statusRealisee,
        series: [
          Series(distance: 10, points: 10, groupSize: 0),
          Series(distance: 10, points: 12, groupSize: 5),
          Series(distance: 10, points: 13, groupSize: 7)
        ],
      );
      final stats = StatsService([s], now: now);
