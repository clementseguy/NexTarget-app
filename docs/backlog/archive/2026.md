# NexTarget — Archive 2026

Cette archive contient les US archivées en 2026, quelle que soit leur date de
livraison ou d'annulation. Les journaux intermédiaires ont été retirés ; les
descriptions, critères d'acceptation et décisions utiles sont conservés.

## Thème 1 — Carnet de tir (Sessions & Séries)

| ID | Titre | Portée | État final | MoSCoW | Est. | Remplacée par |
|---|---|---|---|---|---|---|
| [NT-001](#NT-001) | Enregistrer une session de tir | app | FAIT | Must | M | — |
| [NT-002](#NT-002) | Saisir des séries détaillées | app | FAIT | Must | M | — |
| [NT-003](#NT-003) | Historique & détail des sessions | app | FAIT | Must | M | — |
| [NT-004](#NT-004) | Synthèse libre du tireur par session | app | FAIT | Should | S | — |
| [NT-005](#NT-005) | Attacher une photo de la cible | app | FAIT | Must | M | — |
| [NT-007](#NT-007) | Filtrer l'historique des sessions par exercice | app | FAIT | Could | S | — |
| [NT-008](#NT-008) | Gérer son râtelier d'armes | app | FAIT | Must | M | — |
| [NT-009](#NT-009) | Autocompléter l'arme d'une session depuis le râtelier | app | FAIT | Must | M | — |
...
- **Livraison** : FAIT — `ShootingSession`, `session_service`, `create_session_screen`.
...
- **Livraison** : FAIT — `Series` (enum `HandMethod`), `session_form`.
...
- **Livraison** : FAIT — `sessions_history_screen`, `session_detail_screen`.
...
- **Livraison** : FAIT — `ShootingSession.synthese`.
...
- **Livraison** : FAIT (PR #12, 2026-07-17) — `SessionPhotoService`, `migration_4_add_photo_path_field`, `ShootingSession.photoPath`, `SessionPhotoField` (formulaire) & `SessionPhotoSection` (détail). · **Notes** : préalable à NT-110/NT-111 (et NT-006, Icebox).
...
- **Livraison** : FAIT (PR #12, 2026-07-17) — `SessionFilters.byExercise`, sélecteur d'exercice dans `sessions_history_screen` (combinable avec le filtre réalisées/prévues). · **Notes** : repris de l'issue GitHub #5 (tracking v0.3), 2026-07-09.
...
- **Livraison** : FAIT — livré en v0.6.0 ; `Weapon`, `HiveWeaponRepository` (box `weapons`), `WeaponService` (CRUD, renommage propagé avec rollback), `WeaponRackSection` (`Paramètres > Préférences Tir`), export/import JSON (`BackupService`), `migration_5_create_weapons_box`.
...
- **Livraison** : FAIT — livré en v0.6.0 ; `utils/weapon_autocomplete.dart` (normalisation + suggestions partagées), `WeaponAutocompleteField` réutilisé dans `SessionForm` et le wizard (`WizardIntroStep`).
...
| ID | Titre | Portée | État final | MoSCoW | Est. | Remplacée par |
|---|---|---|---|---|---|---|
| [NT-010](#NT-010) | Tableau de bord statistiques | app | FAIT | Must | M | — |
| [NT-011](#NT-011) | Statistiques explicatives / évolution | app | FAIT | Should | M | — |
| [NT-012](#NT-012) | Objectifs mesurables | app | FAIT | Must | M | — |
| [NT-013](#NT-013) | Hauts faits (records) | app | FAIT | Should | S | — |
| [NT-014](#NT-014) | Comparatif glissant 30j vs 90j + sparkline | app | FAIT | Could | M | — |
| [NT-017](#NT-017) | Compteur de tirs par arme du râtelier | app | FAIT | Should | S | — |
...
- **Livraison** : FAIT — `dashboard_service`, `stats_service`, `widgets/dashboard`.
...
- **Livraison** : FAIT — `docs/features/statistiques.md`, `DashboardService`, `EvolutionChart`.
...
- **Livraison** : FAIT — `Goal` (`GoalMetric`, `GoalComparator`, `GoalStatus`), `goal_service`, écrans list/edit.
...
- **Livraison** : FAIT — enum `GoalMetric` (champs 5-7).
...
- **Livraison** : FAIT — fusionné sur `dev` par la PR #28 le 2026-09-03 ; populations score/groupement indépendantes, fenêtres emboîtées et bornées, deltas absolus/relatifs, carte Progression alignée, états insuffisants, sparklines par session, UI mobile et aide contextuelle.
...
- **Livraison** : FAIT — livré en v0.6.0 ; `DashboardService.generateWeaponShotCounts`, `WeaponShotCountsCard` (dernière section de `Statistiques > Avancé`).
...
| ID | Titre | Portée | État final | MoSCoW | Est. | Remplacée par |
|---|---|---|---|---|---|---|
| [NT-020](#NT-020) | Gérer des exercices (CRUD) | app | FAIT | Must | M | — |
| [NT-021](#NT-021) | Lier exercices ↔ objectifs | app | FAIT | Should | S | — |
| [NT-022](#NT-022) | Lier exercices ↔ sessions | app | FAIT | Should | S | — |
| [NT-023](#NT-023) | Création d'exercice par le coach | both | ANNULÉ | Could | L | NT-122, NT-123 |
| [NT-025](#NT-025) | Niveau de difficulté d'exercice | app | FAIT | Could | M | — |
| [NT-026](#NT-026) | Supprimer un exercice depuis l'interface | app | FAIT | Could | S | — |
| [NT-027](#NT-027) | Dupliquer un exercice | app | FAIT | Could | S | — |
...
- **Livraison** : FAIT — `Exercise`, `exercise_service`, écrans list/form. Ce CRUD
...
- **Livraison** : FAIT — `Exercise.goalIds`.
...
- **Livraison** : FAIT — `ShootingSession.exercises`.

<a id="NT-023"></a>
### NT-023 — Création d'exercice par le coach
- **Annulation** : périmètre remplacé par NT-122, NT-123.
...
- **Notes** : dépend du format de sortie structuré du coach. **Précisé/décomposé par NT-122 + NT-123** (thème 12), qui font référence.
...
- **Livraison** : FAIT — fusionné sur `dev` par la PR #34 le 2026-09-05.
...
| ID | Titre | Portée | État final | MoSCoW | Est. | Remplacée par |
|---|---|---|---|---|---|---|
| [NT-030](#NT-030) | Analyse d'une session par le coach IA | both | FAIT | Must | M | — |
| [NT-031](#NT-031) | Prompt d'analyse centralisé côté serveur | server | FAIT | Must | S | — |
| [NT-032](#NT-032) | Multi-personas coach (neutre / cool) | both | FAIT | Should | M | — |
| [NT-033](#NT-033) | Écran "Coach" : analyse transverse multi-sessions | both | ANNULÉ | Should | L | NT-120, NT-121 |
...
- **Livraison** : FAIT — serveur `api/coach.py` ; app `ServerCoachAnalysisService` (unique chemin d'analyse depuis NT-061).
...
- **Livraison** : FAIT — `services/prompt_builder.py`.
...
- **Livraison** : FAIT (2026-07-07, sprint S2) — serveur : `coach_cool.yaml` + `_VARIANT_FILES` ; app : préférence `coach_persona` (Paramètres > Coach IA), envoyée en `prompt_variant`.
...
- **Annulation** : périmètre remplacé par NT-120, NT-121.
...
| ID | Titre | Portée | État final | MoSCoW | Est. | Remplacée par |
|---|---|---|---|---|---|---|
| [NT-040](#NT-040) | Authentification OAuth Google | both | FAIT | Must | M | — |
| [NT-041](#NT-041) | Authentification optionnelle | app | FAIT | Must | S | — |
| [NT-042](#NT-042) | Profil utilisateur (nom/avatar/niveau) | both | FAIT | Should | M | — |
| [NT-043](#NT-043) | Endpoint `/users/me` | server | FAIT | Must | S | — |
| [NT-048](#NT-048) | Refresh tokens + rotation | both | FAIT | Should | M | — |
| [NT-049](#NT-049) | Interface d’administration read-only des utilisateurs | server | FAIT | Should | M | — |
...
- **Livraison** : FAIT — serveur `api/auth_google.py`, app `auth_service.dart`.
...
- **Livraison** : FAIT — `auth_provider`, mode déconnecté préservé.
...
- **Livraison** : FAIT — `models/user.py`, `profile_screen.dart`.
...
| ID | Titre | Portée | État final | MoSCoW | Est. | Remplacée par |
|---|---|---|---|---|---|---|
| [NT-050](#NT-050) | SonarCloud + Quality Gate + couverture (app) | app | FAIT | Must | M | — |
| [NT-051](#NT-051) | Analyse statique & lint (durcir) | app | FAIT | Should | S | — |
| [NT-052](#NT-052) | Cahier de recette généré | app | FAIT | Should | S | — |
| [NT-053](#NT-053) | Logging structuré + tracing (serveur) | server | FAIT | Should | M | — |
| [NT-054](#NT-054) | Tests OAuth mockés (providers externes) | server | FAIT | Should | M | — |
| [NT-055](#NT-055) | CI serveur (tests + couverture) | server | FAIT | Should | S | — |
| [NT-056](#NT-056) | Harmonisation des erreurs réseau (app) | app | FAIT | Could | M | — |
| [NT-057](#NT-057) | Nettoyage des widgets dupliqués (app) | app | FAIT | Could | S | — |
| [NT-058](#NT-058) | Fakes de repository partagés pour les tests (app) | app | FAIT | Should | S | — |
| [NT-059](#NT-059) | Résorber les issues SonarCloud de la PR #32 et les tests ignorés | app | FAIT | Should | M | — |
...
- **Livraison** : FAIT — `.github`, `sonar-project.properties`, CHANGELOG T1.
...
- **Livraison** : FAIT (2026-07-07, sprint S1) — `flutter_lints` activé (`analysis_options.yaml`), 138 issues corrigées (dont un vrai bug : route `/settings` jamais résolue, `unrelated_type_equality_checks`), step CI `flutter analyze --fatal-infos` ajouté au workflow SonarCloud.
...
- **Livraison** : FAIT — `docs/tests/cahier_recette.*`.

<a id="NT-053"></a>
### NT-053 — Logging structuré + tracing (serveur)
- **Portée** : server · **Dépendances** : — · **Description** : Observabilité serveur (JSON + OpenTelemetry).
- **Critères d'acceptation** : logs structurés ; corrélation des requêtes. · **Livraison** : FAIT (2026-07-09, sprint S3) — logs JSON (stdlib) + middleware X-Request-ID (une ligne par requête : method/path/status/durée). OpenTelemetry écarté en single-instance (décision documentée AGENTS serveur).
...
- **Critères d'acceptation** : Google/Facebook mockés ; cas nominal + erreurs. · **Livraison** : FAIT (2026-07-09, sprint S3) — `tests/test_oauth_flows.py` (flows complets Google/Facebook mockés, nominal + erreurs), fixtures partagées `tests/conftest.py`, migration ASGITransport.
...
- **Livraison** : FAIT (2026-07-09, sprint S3) — `.github/workflows/ci.yml` (pytest + pytest-cov, Python 3.11, push/PR).
...
- **Livraison** : FAIT — `test/support/fake_session_repository.dart`, `test/support/async_test_helpers.dart` (+ tests dédiés) ; `test/goal_service_lot_a_test.dart` migré. Convention documentée dans `AGENTS.md`.
...
| ID | Titre | Portée | État final | MoSCoW | Est. | Remplacée par |
|---|---|---|---|---|---|---|
| [NT-060](#NT-060) | Proxy Mistral côté serveur (clé hors client) | server | FAIT | Must | M | — |
| [NT-061](#NT-061) | Coach « connecté uniquement » : retrait clé Mistral client + rotation | both | FAIT | Must | M | — |
| [NT-062](#NT-062) | Rate limiting de l'endpoint coach | server | FAIT | Must | S | — |
| [NT-063](#NT-063) | State OAuth à usage unique (CSRF) | server | FAIT | Must | S | — |
| [NT-064](#NT-064) | Vérification du type de token JWT | server | FAIT | Must | S | — |
| [NT-065](#NT-065) | Restreindre CORS par environnement | server | FAIT | Should | S | — |
| [NT-066](#NT-066) | Vérification du nonce Google | server | FAIT | Should | S | — |
...
- **Livraison** : FAIT — `api/coach.py`, `services/mistral_client.py`, `core/config.py`.
...
- **Livraison** : FAIT — `services/rate_limiter.py` (in-memory). · **Notes** : Redis si multi-instance (lié à NT-071).
...
- **Livraison** : FAIT — `services/oauth_state.py`. · **Notes** : en mémoire (single-instance).
...
- **Livraison** : FAIT — `core/security.py`, `api/deps.py`.

<a id="NT-065"></a>
### NT-065 — Restreindre CORS par environnement
- **Portée** : server · **Dépendances** : — · **Description** : `allow_origins=["*"]` est un TODO connu.
- **Critères d'acceptation** : origines restreintes en prod via configuration. · **Livraison** : FAIT (2026-07-07, sprint S1) — `CORS_ALLOW_ORIGINS` (défaut : `*` en dev, aucune origine sinon), tests `tests/test_cors.py`.
...
- **Critères d'acceptation** : nonce vérifié à la réception du callback. · **Livraison** : FAIT (2026-07-07, sprint S1) — claim `nonce` comparé au state stocké (400 sinon), tests mockés `tests/test_auth_google_nonce.py`.
...
| ID | Titre | Portée | État final | MoSCoW | Est. | Remplacée par |
|---|---|---|---|---|---|---|
| [NT-070](#NT-070) | Déploiement serveur (Render) | server | FAIT | Must | S | — |
| [NT-071](#NT-071) | Migration SQLite → Postgres Neon + Alembic | server | FAIT | Must | M | — |
| [NT-072](#NT-072) | Framework de migrations Hive | app | FAIT | Should | M | — |
| [NT-073](#NT-073) | Calibre par défaut + normalisation statistique | app | FAIT | Could | S | — |
| [NT-075](#NT-075) | Onboarding + aide contextuelle | app | FAIT | Could | M | — |
| [NT-077](#NT-077) | Vérifier automatiquement la cohérence du schéma Hive | app | FAIT | Should | S | — |
...
- **Livraison** : FAIT — `render.yaml`, `docs/tech/render_setup.md`.
...
- **Livraison** : FAIT — livré dans la release produit v0.6.0 (composant NexTarget-server v0.3.0) ; PostgreSQL Neon, Alembic, migrations avant démarrage, rôles runtime/propriétaire séparés et procédures de sauvegarde/rollback documentés.
...
- **Livraison** : FAIT — `lib/migrations/` (`MigrationRunner`, `SchemaVersionStore`, migrations 2 & 3).
...
- **Livraison** : FAIT — livré sur `dev` par la PR #26 le 2026-09-03 ; critères validés par la suite Flutter complète et la recette manuelle.
- **Notes** : un prototype commun avec NT-100/101/130 a été abandonné le 2026-07-24 après recette UX ; respecter le [REX TAR & saisie rapide](../details/rex-tar-saisie-rapide-2026-07-24.md). Le préremplissage ne doit ajouter aucune étape au parcours classique.
...
- **Critères d'acceptation** : onboarding au 1er lancement ; aide sur Objectifs/Exercices/Sessions. · **Livraison** : FAIT (2026-07-07, sprint S2) — `OnboardingScreen`/`OnboardingGate` (3 écrans, flag `onboarding_seen`, ré-accès via Paramètres > Aide), `HelpButton` sur Sessions, Objectifs, Exercices (+ hub Exercices & Objectifs).
...
| ID | Titre | Portée | État final | MoSCoW | Est. | Remplacée par |
|---|---|---|---|---|---|---|
| [NT-092](#NT-092) | Thèmes visuels (thème clair « France ») | app | FAIT | Could | S | — |

<a id="NT-092"></a>
### NT-092 — Thèmes visuels (thème clair « France »)
- **Portée** : app · **Description** : Thématisation de l'app, dont le thème clair « France ». · **Livraison** : FAIT — commit `feat: thème clair France`. · **Notes** : d'autres thèmes possibles (voir NT-090).
...
| ID | Titre | Portée | État final | MoSCoW | Est. | Remplacée par |
|---|---|---|---|---|---|---|
| [NT-131](#NT-131) | Session guidée au stand | app | FAIT | Should | L | — |
| [NT-133](#NT-133) | Sessions libres sans séries ni scores | app | FAIT | Must | L | — |
...
- **VM** : 4 · **Livraison** : FAIT — fusionné sur `dev` par la PR #31 le 2026-09-04, après corrections de revue et de recette.
...
- **VM** : 4 · **Livraison** : FAIT — fusionné sur `dev` par la PR #27 le 2026-09-03.
- **Notes** : `SimpleShootingSession` est le nom technique retenu ; « Session libre » est exclusivement le label utilisateur. Le champ de commentaires réutilise la notion et le libellé de **synthèse** des sessions détaillées. Préserver le parcours actuel du `+` est non négociable pour la livraison autonome de NT-133, conformément au [REX TAR & saisie rapide](../details/rex-tar-saisie-rapide-2026-07-24.md) ; l'organisation des actions pourra ensuite être remplacée par le parcours validé de NT-131.

## Thème 14 — Finitions UX

| ID | Titre | Portée | État final | MoSCoW | Est. | Remplacée par |
|---|---|---|---|---|---|---|
| [NT-140](#NT-140) | Supprimer le second écran de chargement Flutter | app | FAIT | Should | S | — |
| [NT-141](#NT-141) | Réordonner les sections de l'écran Paramètres | app | FAIT | Should | S | — |
| [NT-142](#NT-142) | Déplacer « Tirs par arme » en bas de Synthèse | app | FAIT | Should | S | — |
| [NT-143](#NT-143) | Remplacer « Copier résumé » par la duplication de session | app | FAIT | Should | M | — |
| [NT-144](#NT-144) | Assainir et réorganiser la documentation du backlog | app | FAIT | Could | S | — |
| [NT-145](#NT-145) | Ouvrir la session associée à un record | app | FAIT | Should | S | — |
| [NT-146](#NT-146) | Aider à estimer la taille d'un groupement | app | FAIT | Could | S | — |
| [NT-147](#NT-147) | Déplacer « Répartition Catégories » sous les calibres | app | FAIT | Should | S | — |
| [NT-148](#NT-148) | Séparer les trois préférences de tir | app | FAIT | Could | S | — |
| [NT-149](#NT-149) | Renommer les fichiers d'export NexTarget | app | FAIT | Should | S | — |
| [NT-150](#NT-150) | Garantir un état d'authentification cohérent après mise à jour | app | FAIT | Should | M | — |
...
- **VM** : 2 · **Livraison** : FAIT — fusionné sur `dev` par la PR #34 le 2026-09-05.
...
- **VM** : 3 · **Livraison** : FAIT — fusionné sur `dev` par la PR #34 le 2026-09-05.
...
- **VM** : 3 · **Livraison** : FAIT — fusionné sur `dev` par la PR #36 le 2026-09-07.
...
- **VM** : 2 · **Livraison** : FAIT — fusionné sur `dev` par la PR #36 le 2026-09-07.
...
- **VM** : 1 · **Livraison** : FAIT — fusionné sur `dev` par la PR #36 le 2026-09-07.
