...
- **Notes** : NT-154 est autonome pour la saisie et la persistance locale. NT-156 porte seule l'intégration serveur de ces données, ce qui supprime toute dépendance réciproque. Le débrief structuré `coachAnalysis` remplace définitivement l'ancien champ Markdown `analyse`, converti par migration ou import en débrief historique structuré ; l'indicateur de la liste des sessions repose sur ce débrief structuré.

<a id="NT-157"></a>
### NT-157 — Séparer décision et ton du Coach de session
- **Thème** : Socle Coach transverse · **Portée** : server · **Dépendances** : NT-032, NT-122, NT-156, NT-163
...
- **Critères d'acceptation** : le moteur NT-163 produit une sortie structurée validée contenant les métriques et décisions immuables ; un prompt reçoit cette sortie et la reformule selon le ton configuré ; la reformulation ne peut modifier aucune métrique, conclusion, évaluation ou prochaine action ; les personas partagent donc exactement la même décision ; si la reformulation échoue ou devient invalide, le serveur renvoie une formulation de secours issue de la sortie structurée.
- **Notes** : pipeline attendu : moteur déterministe NT-163 → sortie structurée → `PromptTonCoach` → texte utilisateur.
...
<a id="NT-159"></a>
### NT-159 — Créer le modèle Exercise partagé et sa provenance
- **Thème** : Socle Coach transverse · **Portée** : both · **Dépendances** : NT-020, NT-025, NT-072, NT-152
- **Description** : Créer côté serveur le modèle et les schémas Exercise alignés sur le modèle complet existant dans l'app, puis ajouter au contrat partagé la provenance nécessaire pour distinguer les exercices personnels locaux de ceux issus du catalogue serveur.
- **Critères d'acceptation** : le domaine et les schémas serveur représentent tous les champs Exercise existant dans l'app au démarrage de l'US, avec des types, valeurs contrôlées, noms d'échange et règles de nullabilité explicitement alignés ; le modèle app et le serveur ajoutent une provenance contrôlée `personal` ou `coach_catalog` ; les exercices existants et importés sans provenance deviennent `personal` sans intervention utilisateur ; l'identifiant `id` existant reste l'identifiant commun et aucun `serverId` n'est ajouté ; un exercice personnel conserve son identifiant généré localement et un exercice du catalogue utilise l'identifiant fourni par le serveur ; sérialisation, désérialisation réseau, duplication, import, export et migrations préservent la provenance ; des tests de contrat vérifient la compatibilité du modèle complet et des valeurs d'énumération entre app et serveur.
- **Notes** : NT-159 définit le modèle partagé et prépare son transport, mais ne crée ni table, ni migration Alembic, ni endpoint de catalogue. NT-160 porte la persistance du catalogue, son API et sa synchronisation serveur vers app. Un exercice personnel reste local et n'est jamais synchronisé comme entité serveur ; son instantané ponctuel pendant une analyse relève de NT-161. Le seul champ ajouté au modèle app existant est la provenance : aucun autre champ métier, version d'exercice, statut de catalogue ou métadonnée de validation n'est ajouté ici. La provenance interne `coach_catalog` est présentée à l'utilisateur comme « Créé par le Coach », sans signifier une génération dynamique par l'IA. Les enrichissements métier sont suivis par NT-162.

<a id="NT-160"></a>
### NT-160 — Récupérer à la demande les exercices du catalogue Coach
- **Thème** : Socle Coach transverse · **Portée** : both · **Dépendances** : NT-159
- **Description** : Faire du serveur la source de vérité d'un catalogue préparé et validé, puis permettre à l'app de récupérer uniquement les exercices qui lui sont attribués par leur identifiant, sans télécharger le catalogue complet.
- **Critères d'acceptation** : le serveur persiste uniquement les exercices de provenance `coach_catalog` définis par NT-159 et ajoute seulement un statut technique `is_active`, vrai par défaut ; le catalogue de production peut être vide et les tests utilisent des fixtures techniques ; une API en lecture seule permet de récupérer par son identifiant stable un exercice actif, sans endpoint client de parcours complet, création, modification ou suppression ; l'app télécharge ou actualise uniquement un exercice explicitement demandé, conserve cette copie locale hors ligne et ne transmet jamais d'exercice vers le catalogue ; une réponse absente, inactive, invalide ou une erreur réseau ne supprime ni l'exercice déjà en cache ni les exercices personnels ; listes et détails affichent « Créé par le Coach » et un exercice Coach n'est ni modifiable ni supprimable depuis l'app ; aucune récupération automatique n'a lieu à l'ouverture de la liste ; jusqu'à l'arrivée du Coach de progression, un contrôle placé dans la barre supérieure de la liste des exercices, visible uniquement en mode DEBUG comme le bouton éclair des sessions, permet de saisir un identifiant et de déclencher ce téléchargement ciblé ; les tests couvrent persistance, filtrage des éléments inactifs, lecture par identifiant, absence d'API interdite, mise à jour et conservation du cache, isolation des exercices personnels, affichage, refus des mutations et visibilité DEBUG du contrôle.
- **Notes** : la future attribution par le Coach de progression déclenchera le même mécanisme de récupération ciblée que le contrôle DEBUG. La synchronisation du catalogue reste strictement serveur vers app et ne dépend pas du consentement d'envoi de données aux Coachs. Aucun exercice réel, workflow éditorial, métadonnée de validation, versionnement ou synchronisation incrémentale n'est introduit. L'instantané historique utilisé par une session ou une analyse relève de NT-156. « Créé par le Coach » désigne un contenu humainement préparé et validé, jamais généré à la volée.
...
- **Critères d'acceptation** : un expert valide les informations nécessaires pour comprendre, réaliser et évaluer un exercice ; un dictionnaire partagé fixe noms, types, unités, caractère obligatoire, valeurs contrôlées et règles de validation avant le développement ; les champs candidats incluent prérequis, protocole structuré, compétence ou objectif travaillé, durée, nombre attendu de séries et de coups, contraintes de cible, calibre et distance, mesure ou KPI, agrégat, unité, sens d'amélioration, comparateur, valeur cible, nombre minimal de séries et de tirs valides, critère de réussite, éventuelle déclinaison par niveau, variante simplifiée, limites et consignes de sécurité, version, source et traçabilité de validation ; aucun seuil lié au niveau n'est retenu sans validation métier explicite ; seuls les champs retenus après expertise sont implémentés ; un exercice sans critère mesurable reste réalisable et analysable mais sa performance ne peut pas être déclarée satisfaisante ; un exercice personnel sans critère explicite reste évalué uniquement sur sa réalisation et le suivi déclaré du protocole ; les règles et seuils sont versionnés hors du prompt ; les schémas app et serveur restent alignés par des tests de contrat ; les migrations Hive et Alembic sont additives et rétrocompatibles ; les exercices historiques restent lisibles sans valeurs inventées.
- **Notes** : cette US est volontairement lancée après NT-154 et NT-156. Elle ne bloque pas le débrief initial, qui évalue seulement la réalisation et le suivi du protocole déclarés. Elle fournit le contrat métier nécessaire au moteur d'évaluation NT-163 ; la recherche de sources peut aider à préparer le référentiel, mais leur intégration exige une validation humaine et aucune recherche web en direct n'entre dans la décision.

<a id="NT-163"></a>
### NT-163 — Déterminer la réussite et la suite du Coach de session
- **Thème** : Socle Coach transverse · **Portée** : both · **Dépendances** : NT-156, NT-162
- **Description** : Évaluer de façon stable et explicable la tentative d'un exercice à partir des données de la session et du critère mesurable de l'exercice, puis déterminer la seule prochaine action autorisée sans déléguer cette décision au modèle de langage.
- **Critères d'acceptation** : le serveur calcule de manière déterministe les métriques nécessaires à partir des séries valides et conserve les valeurs, unités et règles utilisées ; l'évaluation distingue au minimum réalisation, suivi du protocole, performance satisfaisante, insuffisante, irrégulière ou non évaluable, résultat de l'exercice, niveau de confiance, preuves, limites et code de décision ; un exercice est réussi seulement s'il est réalisé, si son protocole est suivi et si son critère mesurable est atteint ; un exercice non réalisé, un protocole non ou partiellement suivi, un critère absent ou des données insuffisantes rendent la performance non évaluable et ne permettent pas d'attribuer les résultats de la session à l'exercice ; un critère non atteint ou une performance irrégulière après une tentative évaluable conduit à refaire l'exercice ; une réussite conduit à demander une nouvelle décision au Coach de progression ; en l'absence d'historique de tentatives, une tentative non réussie ne déclenche jamais seule une réévaluation longitudinale ; le résultat et la prochaine action sont calculés avant l'appel IA, revalidés après celui-ci et identiques pour toutes les personas ; le modèle peut uniquement expliquer la décision et distinguer mesures objectives et déclarations du tireur ; le fallback renvoie la même décision déterministe ; l'analyse persistée conserve les métriques, preuves, limites, code de décision et versions des règles ; l'application affiche ces éléments sans transformer `not_evaluable` en échec ; les tests couvrent chaque branche de la matrice, les données manquantes ou incohérentes, les exercices personnels sans critère et l'identité de décision entre personas.
- **Notes** : le niveau d'expérience adapte la formulation et peut sélectionner un critère explicitement validé pour ce niveau, mais ne constitue jamais à lui seul une norme de réussite. Le Coach de session ne choisit ni nouvel axe, ni nouvel objectif, ni nouvel exercice. Les transitions fondées sur plusieurs tentatives restent réservées au Coach de progression et à NT-125.

<a id="NT-164"></a>
### NT-164 — Gérer l'attente du Coach de progression
- **Thème** : Socle Coach transverse · **Portée** : app · **Dépendances** : NT-156, NT-163
